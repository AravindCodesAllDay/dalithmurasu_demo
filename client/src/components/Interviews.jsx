...
const API_BASE = "http://localhost:3030/api/interviews"; // change if different
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      ...
      if (res.ok) {
        ...
        setEditingId(null);
        setFormData({ title: "", date: "", category: "", photo: "", parts: [] });
        fetchInterviews();
      }
    } else {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "auto" }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: 20 }}>
        ...
        <button type="submit">{editingId ? "Update" : "Create"} Interview</button>
        {editingId && (
          <button
            type="button"
            onClick={() => {
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {interviews.map((intv) => (
          ...
          >
            ...
            {intv.photo && <img src={intv.photo} alt={intv.title} style={{ width: 150 }} />}
            <br />
            <button onClick={() => handleEdit(intv)} style={{ marginRight: 10 }}>
              Edit
            </button>
