...
export default function PeriyarSpeaksDetail() {
  const { id } = useParams();
  const article = periyar_speaks.find((a) => a.id === id);
  ...
  const currentIndex = periyar_speaks.findIndex((a) => a.id === id);
  const prevArticle = periyar_speaks[currentIndex - 1];
  const nextArticle = periyar_speaks[currentIndex + 1];
  ...
  return (
    <div className="bg-white rounded-lg p-6 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between pt-6 border-t mt-8">
        {prevArticle ? (
          <Link
            to={`/periyar_speaks/${prevArticle.id}`}
        ...
        {nextArticle ? (
          <Link
            to={`/periyar_speaks/${nextArticle.id}`}
            className="px-4 py-2 border border-highlight-1 hover:bg-highlight-1 hover:text-white rounded transition-all duration-150 ease-in-out"
          >
            Next →
          </Link>
